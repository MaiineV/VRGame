...
namespace Gameplay.Systems
{
    ...
    public sealed class RuntimeNavMeshBaker : MonoBehaviour
    {
        ...
        /// <summary>Rebuild the navmesh from the current scene geometry. Safe to call again (replaces).</summary>
        public void Bake()
        {
            ...
            // BuildNavMeshData expects bounds in the local space of (position, rotation).
            var localBounds = new Bounds(Vector3.zero, _bounds);
            var data = NavMeshBuilder.BuildNavMeshData(settings, _sources, localBounds, transform.position, Quaternion.identity);

            if (_instance.valid) NavMesh.RemoveNavMeshData(_instance);
            _instance = NavMesh.AddNavMeshData(data);

            MyLogger.LogInfo($"[RuntimeNavMeshBaker] Baked navmesh from {_sources.Count} source(s) (radius={_agentRadius}, valid={_instance.valid}).");
        }
